...
Fetcher.prototype._listMinistries = function _listMinistries(done) {
  this._fetchBodyBox(constants.ORM_MINISTRIES, {}, function(err, bodyBox) {
    if (err) {
      return done(err);
    }
...
Fetcher.prototype._listMinistryDocuments = function _listMinistryDocuments(ministryId, documentType, limit, done) {
  // TODO Iterate over multiple pages
  this._fetchBodyBox(constants.FORM_MINISTRY, {res: ministryId, nres: documentType}, function(err, bodyBox) {
    if (err) {
      return done(err);
    }
...
Fetcher.prototype._parseDocumentElements = function _parseDocumentElements(elements, done) {
  ...
  var _newSubParagraph = function _newSubParagraph(id, no, text) {
    if ( id && isMainChapters ) {
      var chapterIndex = result.chapters.length - 1;
      var paragraphIndex = result.chapters[chapterIndex].paragraphs.length - 1;

      result.chapters[chapterIndex].paragraphs[paragraphIndex].subParagraphs.push({
        id: id,
        no: no,
        text: text,
      });
  ...
  var _newParagraph = function _newParagraph(id, no, text) {
    if ( result.chapters.length ) {
      ...
      result.chapters[chapterIndex].paragraphs.push({
        id: id,
        no: no,
        text: text,
        subParagraphs: []
  ...
  elements.forEach(function parseSingleElement(el) {
    ...
    // After new chapter, comes the chapter title
      ...
      result.chapters[result.chapters.length-1].title = el.children[0].data;
    }

    // New paragraph inside current chapter
      ...
      _newParagraph(id, no, textElements[2].data.trim());

    }

    // New sub paragraph (Stk) inside current paragraph
    ...
    // Signature part "Givet"
      ...
      if ( typeof ministryAndDate === 'Error' ) {
 ...
 * @param {integer} ministryId ID as returned from listMinistries
 * @param {integer} documentType Document type (1-6)
 * @param {integer} limit (Optional) Maximum number of documents to list
 * @param {function} done Callback(err, documents)
 */
